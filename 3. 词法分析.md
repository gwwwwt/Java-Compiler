...
> **参考：[Unicode 编码及 UTF-32, UTF-16 和 UTF-8](https://zhuanlan.zhihu.com/p/51202412)**

> **UTF-16编码：**
>
> **Unicode字符集采用两个字节来编码一个字符，而UTF-16编码就是直接采用了这固定的的两个字节，内存表现方式比较简单、直接。**
>
> > ***注：由于Unicode包含的字符越来越多，导致两个字节已经无法涵盖所有字符了。出现的结果就是UTF-16也变成了变长编码，但是只有2个字节或者4个字节的可能，具体参考相关资料。***
>
...
> ***注：Java在生成Class文件时，字符常量会使用一种改进的UTF-8编码(modified UTF-8)， 这种改进的编码会使用上面标准的UTF-8编码中的单字节、双字节和三字节格式。因为三字节格式有效的编码位为16位，所以多于三个字节的位要使用更的字节来表示。对于UTF-16编码来说，多于16位的会使用2个16位来表示，而改进的UTF-8编码在处理超过16位表示范围的字符时，采取和UTF-16编码类似的处理方式，以代理对的形式表示。***
...
public class SharedNameTable extends Name.Table {
    ...
    @Override public Name fromChars(char[] cs, int start, int len) {
        ...
        //由于是char[]数组, 每个char 2个字节, 所以转换成Utf8字节编码时不需要考虑代理对的问题
...
#### 3.3.1 SharedNameTable和Name实例包装类：Names

> **Names构造方法创建SharedNameTable实例，并用通用字符串构造了多个Name实例作为成员变量。**
>
> **此外，Names定义了注册与获取Name实例的多个 `fromXXX` 方法，作为其它模块调用的接口。**
...
private void scanChar() {
        ch = buf[++bp];
        if (ch == '\\') { convertUnicode(); } //对于需要转换Unicode的情况
}

//涉及到的成员变量
private int unicodeConversionBp = -1;
private void convertUnicode() { //解析可能的十六进制unicode字符 \uXXXX
        if (ch == '\\' && unicodeConversionBp != bp) {
            bp++; ch = buf[bp];
            //如果'\'符号后不是'u'则直接返回, 符号就是一个'\'
            //如果是'u'则再解析后面四个十六进制值
            //成员变量 unicodeConversionBp指向'\uXXXX'第一个X索引
            if (ch == 'u') { 
                do {
                    bp++; ch = buf[bp];
                } while (ch == 'u');
            ...
            } else { //如果'\'后不是'u', 则bp回退到'\'后的第一个字符, ch值也是'\'符号
                	//这里就不按java语法去转义'\'了
                bp--;
                ch = '\\';
            }
...
### 4.2  Scanner#nextToken() 实现

```java
//sbuf用于缓存读取的符号, 如果符号字符串长度大于128, 会扩容数组长度
private char[] sbuf = new char[128];
private int sp; //标识sbuf中的可写入位置, 在每次调用nextToken()时都会重置为0

private char[] buf; //保存整个Java源文件内容
private int bp;     //已读取到buf中的位置
private int buflen; //文件字符长度
private int eofPos; //Character position just after the last character of the token
...
//Jdk是否支持使用代理对表示Unicode字符, 默认是true, 应该是支持的;
//因为如果支持代理对的话, 就需要在解析文件符号时处理可能的代理对表示的字符
        ...
        try {
            ...
            while (true) {
                ...
                case FF:
                    ...
                    endPos = bp; //跳过连续的空白符号, endPos指向空白符后面第一个非空白字符
                    processWhiteSpace();
                    break;
                case LF: //换行符
                    scanChar();
                ...
                //--- 8. 默认的处理        
                default:
                    if (isSpecial(ch)) { //处理'@', '!','?'等符号, 这些符号本身也对应一个Token
                        scanOperator();
                    } else {
...
```java
private void scanIdent() { //com.sun.tools.javac.parser.Scanner; 获取标识符 
        boolean isJavaIdentifierPart;
        char high;
        do {
            //如果sbuf缓存区已满, 则扩容sbuf后再添加ch; 如果未满, 则直接在sbuf中添加ch; 
            //并递增sp值
            if (sp == sbuf.length) putChar(ch); else sbuf[sp++] = ch;

            scanChar(); //读取下一个字符
            switch (ch) {
            //如果下一个字符是合法字符, 则将其加入到sbuf数组后继续读取下一个字符        
            case 'A': case 'B': case 'C': case 'D': case 'E':
            case 'F': case 'G': case 'H': case 'I': case 'J':
            case 'K': case 'L': case 'M': case 'N': case 'O':
            case 'P': case 'Q': case 'R': case 'S': case 'T':
            ...
            case '\u001A': // EOI is also a legal identifier part
                //由于标识文件末尾的EOI值也是'0x1A', 但同时它也可以是标识符的一部分
                //所以需要判断这个0x1A是表示到了文件末尾还是一个文件中的字符
                if (bp >= buflen) {
                    //如果读到的0x1A是文件末尾标志, 则直接用sbuf中已有的字符构造Name实例
            ...
            default:
                if (ch < '\u0080') { //ch是ASCII编码中的一个字符
                    //如果ch不是合法的可用于定义变量的字符, 则将isJavaIdentifierPart变量设为false
                    ...
                    if (high != 0) {
                        ...
                        //方法会判断通过高代理和低代理项表示的字符是否为合法的标识符的首字符
