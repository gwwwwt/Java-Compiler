...
public abstract class RelativePath implements Comparable<RelativePath> { //相对路径
    ...
    public static class RelativeFile extends RelativePath {
        ...
        public RelativeFile(String p) {
            super(p);
            if (p.endsWith("/")) {throw new IllegalArgumentException(p);}
        }
        ...
        ZipEntry getZipEntry(ZipFile zip) { //--- 冷不丁来一个这方法,...
...
>
> > **可以指定`useOptimizedZip`命令的值为`false`来使用`ZipArchive`或者`SymbolArchive`类读取压缩包，这两个类在读取压缩包的过程中都使用类库API，实现起来相对简单，读取时也不用关注压缩包的格式等。而`ZipFileIndexArchive`类的读取效率相比`ZipArchive`和`SymbolArchive`这两个类更高，但是实现相对复杂。下面可能主要关注`ZipFileIndexArchive`的实现方式。**
...
public class ZipFileIndex { //com.sun.tools.javac.file.ZipFileIndex
    ...
    ZipFileIndex(File zipFile, RelativeDirectory symbolFilePrefix, boolean writeIndex,
        ...
        //对于ct.sym包, META-INF/sym/rt.jar/; 其余时候为null
    ...
    //--- 核心方法, 读取压缩包的相关内容, 将结果缓存在entries 和 directories成员变量中
    ...
    //---- 了解了上面 directory.buildIndex()的实现逻辑, 以及buildIndex()执行后,
    //---- directories和entries中的结果, 就可以理解下面的readBytes逻辑
    ...
    private byte[] readBytes(Entry entry) throws IOException {
        ...
        //header+26 和 header+28 分别表示Local File header中30字节后
        //额外的File Name和Extra Field字节长度,
        //需要跳过这些长度的字节之后才能读取 Entry对应的File Data
        zipRandomFile.skipBytes(
          get2ByteLittleEndian(header, 26) + get2ByteLittleEndian(header, 28));
    ...
    //参考Entry以及ZipDirectory#buildIndex()方法, 
    //entry.offset是entry对应的Local file header在文件中的偏移
...
static class Entry implements Comparable<Entry> { //表示压缩包中的文件和目录
  ...
  public Entry(RelativePath path) { this(path.dirname(), path.basename()); }
  ...
  public String getName() { return new RelativeFile(dir, name).getPath(); }
  ...
  //根据文件的'Modified time'值转换为long值表示
  ...
  void setNativeTime(int natTime) { nativetime = natTime; }

  public boolean isDirectory() { return isDir; }

  //Entry比较, 先比较dir, 再比较name; 用于Entry[]排序
...
static class DirectoryEntry { //表示压缩包内的相对目录
  ...
  private com.sun.tools.javac.util.List<String> zipFileEntriesFiles = 
    														com.sun.tools.javac.util.List.<String>nil();
     
  private com.sun.tools.javac.util.List<String> zipFileEntriesDirectories =
    														com.sun.tools.javac.util.List.<String>nil();
       
  private com.sun.tools.javac.util.List<Entry>  zipFileEntries = 
    														com.sun.tools.javac.util.List.<Entry>nil();

  private List<Entry> entries = new ArrayList<Entry>(); //本目录下的所有entries, 包括子目录下的entries
  private int numEntries; //entry数量
...
```java
//作用: 读取压缩文件的Central Directory部分, 并且相关信息填充ZipFileIndex的directories和entries字段
private class ZipDirectory { 
  ...
  public ZipDirectory(RandomAccessFile zipRandomFile, long start, long end, ZipFileIndex index) throws IOException {
    ...
    this.zipFileIndex = index;
    hasValidHeader(); //验证是否符合压缩包格式
    findCENRecord(start, end);
  }

  //验证 压缩包 文件内容前4个字符是否是 "PK\0x3\0x4"
  ...
  private void findCENRecord(long start, long end) throws IOException {
    ...
    while (endbufend >= 22) { 
      ...
      //验证方式是: 
      //--- 验证1.找'0x50/0x4b/0x05/0x06'连续的四个字节,
      //--- 验证2.并且需要满足'endbufpos+i+22'+(Comment length字节数)==压缩文件大小
      int i = endbuflen - 22;
      while (i >= 0 &&
             !(endbuf[i] == 0x50 &&
               endbuf[i + 1] == 0x4b &&
      ...
      if (i >= 0) {
        ...
        //zipDir前两个字节保存 "Total entries", 即压缩包中的entry总数量
  ...
  //findCENRecord方法执行后, zipDir数组中保存了Central Directory字节数组内容
  //buildIndex方法则是解析zipDir, 即Central Directory的内容
  //解析具体的File Header Entry时则是调用readEntry方法解析
  private void buildIndex() throws IOException {
    int entryCount = get2ByteLittleEndian(zipDir, 0); //获取2字节的File Header Entry数量

    // Add each of the files
    if (entryCount > 0) {
      //初始化ZipFileIndex#directories为空
      ...
      //--- 将解析到的RelativeDirecotry同样转换为ZipFileIndexEntry后加入到entryList中
      ...
      //结果存储到ZipFileIndex#entries数组, ZipFileIndex#entries中包含文件和目录型Entry
      //并且ZipFileIndex#entries数组将是已排序后结果
  ...
  private int readEntry(int pos, List<Entry> entryList, Map<RelativeDirectory, DirectoryEntry> directories) 
    throws IOException {
...
> **按ZipFileIndexArchive实现, 它将压缩包的相关操作都委派给了`ZipFileIndex类型的zfIndex字段`, 而ZipFileIndex中又将"Central Directory"的解析委派给了`ZipDirectory`负责。**
>
> **`ZipDirectory`解析的结果是 ZipFileIndex.Entry 类型对象，具体根据Entry实例读取压缩包内容并获取实际的文件内容，也是由`ZipFileIndex`实现。 **
>
> **所以实际上ZipFileIndexArchive只是一个包装类，提供了一个对外的调用接口。 **
...
public static class ZipFileIndexFileObject extends BaseFileObject {
  ...
  ZipFileIndexFileObject(JavacFileManager fileManager, ZipFileIndex zfIndex, ZipFileIndex.Entry entry, 
                         File zipFileName) {
    super(fileManager);
  ...
  /*
  对于"ct.sym包", 它的symboleFilePrefix为"META-INF/sym/rt.jar/", 所以结果有前缀
  对于其它文件来说, 直接返回文件名
  */
  ...
  //--- 对于zfIndex.read(entry)获取字节数组参考 1.4节
